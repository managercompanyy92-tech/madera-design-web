...
// === СМЕРТЕЛЬНЫЙ КОД ДЛЯ ПОЛНОЙ ЗАМЕНЫ ФОРМЫ ПРОФИЛЯ ===
(function () {
  function renderLogin() {
    document.querySelector("#app")?.innerHTML = `
      <section class="auth-card">
        <h2>Вход в личный кабинет Madera Design</h2>
        <p>Введите номер телефона и пароль для входа.</p>

        <form id="loginForm">
          <label>Телефон</label>
          <input type="tel" id="phoneLogin" placeholder="+992..." />

          <label>Пароль</label>
          <input type="password" id="passLogin" placeholder="Введите пароль" />
        ...
        <div style="margin-top:12px; font-size:12px; text-align:center;">
          Нет аккаунта?
          <span id="goRegister" style="color:#ffb347; cursor:pointer;">Регистрация</span>
        </div>
      </section>
    `;

    document.querySelector("#goRegister").onclick = renderRegister;
  }

  function renderRegister() {
    document.querySelector("#app")?.innerHTML = `
      <section class="auth-card">
        <h2>Создайте личный кабинет Madera Design</h2>
        <p>Регистрация по номеру WhatsApp. После входа увидите статусы заказов.</p>
        ...
        <form id="registerForm">
          ...
          <label>Телефон</label>
          <input type="tel" id="regPhone" placeholder="+992..." />
        ...
        <div style="margin-top:12px; font-size:12px; text-align:center;">
          Уже есть аккаунт?
          <span id="goLogin" style="color:#ffb347; cursor:pointer;">Войти</span>
        </div>
      </section>
    `;

    document.querySelector("#goLogin").onclick = renderLogin;
  }

  // СЛУШАЕМ ПЕРЕКЛЮЧЕНИЕ СТРАНИЦ SPA
  const origPushState = history.pushState;
  history.pushState = function () {
    origPushState.apply(this, arguments);
    if (location.pathname.includes("/profile")) {
      setTimeout(renderLogin, 50);
    }
  };
  ...
  // ЕСЛИ мы уже на странице /profile — сразу включаем Вход
  if (location.pathname.includes("/profile")) {
    setTimeout(renderLogin, 50);
  }
})();
